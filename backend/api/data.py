...
@router.get("/realtime/{device_id}", response_model=List[ProductionDataResponse])
def get_realtime_data(
    device_id: int, limit: int = Query(20, ge=1, le=200), db: Session = Depends(get_db)
):
    return (
        db.query(ProductionData)
        .filter(ProductionData.device_id == device_id)
        .order_by(ProductionData.timestamp.desc())
        .limit(limit)
        .all()
    )
